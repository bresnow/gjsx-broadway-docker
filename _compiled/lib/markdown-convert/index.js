...
import {
  getParagraphs,
  convertToObject,
} from "./converter-utilities/deassembling.js";
...
import {
  findListIndexes,
  markListItems,
  findOrderedListItems,
} from "./converter-utilities/lists.js";
import {
  addContainerDivs,
  combineText,
} from "./converter-utilities/reassembling.js";
export default function convertToHTML(markdownText) {
  let paragraphs = getParagraphs(markdownText);
  let paragraphObjects = paragraphs.map((paragraph) =>
    convertToObject(paragraph)
  );
  ...
  paragraphObjects = paragraphObjects.map((paragraph) => {
    return {
      type: paragraph.type,
      content: convertText(paragraph.content + " ").trim(),
    };
  });
