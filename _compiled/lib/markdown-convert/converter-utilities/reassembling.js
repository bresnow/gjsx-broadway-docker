export function addContainerDivs(objectArr) {
  ...
  objectArr.forEach((item, index) => {
    if (parentOptions[0].includes(item.type)) {
      let parentIndex = parentOptions[0].indexOf(item.type);
      if (
        !objectArr[index - 1] ||
        objectArr[index - 1].type !== parentOptions[0][parentIndex]
      ) {
        result.push({
...
export function combineText(objectArr) {
  return objectArr.map((item, index) => {
    if (item.type === "hr" || item.type === "br") {
      ...
      if (
        (objectArr[index + 1] && objectArr[index + 1].type === "ol li") ||
        objectArr[index + 1].type === "ul li"
      ) {
