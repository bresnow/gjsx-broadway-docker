...
const WebBrowser = GObject.registerClass(
  class WebBrowser extends Gtk.Application {
    ...
    _init() {
      ...
      let title, url, array = ARGV

      for (let i = 0; i < array.length; i++) {
        const arg = array[i], argv = array[i + 1];
        switch (arg) {
          case 'title':
            title = argv;
            break;
          case 'url':
            ...
            break;
          default:
            title = "Floating Mammoth Web Viewer";
            url = "fltngmmth.com"
            break;
      ...
      _buildUI() {
        ...
        this._webView.get_settings().set_enable_developer_extras(true);
        this._webView.get_settings().zoom_level(120)
        this._webView
          .get_settings()
